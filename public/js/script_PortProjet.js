$(function() {
	$('select[name="typeOrganisation"]').change(function() {
		...
		if (valeur == 'particulier') {
			...
			var select = $(
				'<select name="Nom" class="custom-select col" id="nomentreprise"><option value="" selected>Choisissez le nom de votre Entreprise</option><option value="apple">Apple</option><option value="facebook">Facebook</option><option value="ajouter">Ajouter</option></select>'
			);
			...
			$('#posteEntreprise').prop('required', false);
			var select = $(
				'<select name="Nom" class="custom-select col" id="nomassociation"><option value="" selected>Choisissez le nom de votre Association</option><option value="connectanou">Connectanou</option><option value="croixrouge">Croix Rouge</option><option value="ajouter">Ajouter</option></select>'
			);
			$('#Organisation').append(select);
